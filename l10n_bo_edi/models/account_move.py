...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        #@ivan_porras: Ikatech Code
        for record in self:
            if record.country_code == 'BO':
                if not cancel_flag:
                    if (record.journal_id.type == 'sale' or record.journal_id.type == 'purchase'):
                        if record.inv_type: ## Si es factura de venta
                            if record.e_billing: ## Si la modalidad es electronica...
                                if not record.external_con: ## Si la conexion al SIN será por Odoo o por backend .NET
                                    ...
                                    if record.is_cancelled:
                                        ...
                                        res_conectivity = record.check_conectivity()
                                        if res_conectivity:
                                            record.is_confirmed = True
                                            current_status = int(record.env['bo_edi_params'].search(
                                                    [('name', '=', 'ONLINE')]).value)
                                            internet_status = int(record.env['bo_edi_params'].search(
                                                    [('name', '=', 'DEMO_INT_STATUS')]).value)

                                            if current_status:
                                                record.send_email_with_attachment(0)
                                            elif internet_status:
                                                record.send_email_with_attachment(1)

                                            record.print_report()
                                else:
                                    new_inv = record.post_invoice(0)
                                    if new_inv[0]:
                                        record.send_email_with_attachment(0 , new_inv[1][0]["xml"])
                                    elif new_inv[0] == 0:
                                        raise ValidationError("The current invoice is already posted in SIN")

                            else: ## Si la modalidad es estandar
                                nit = record.partner_id.vat
                                currency_name = record.invoice_line_ids.mapped('sale_line_ids').order_id.pricelist_id.currency_id.name
                                if not nit:
                                    raise ValidationError("There is no VAT/NIT for the client company")
                                else:
                                    record.generate_control_code()
                                    record.is_confirmed = True
                                    record.print_report()
